...
yaml_template = '/home/yu79deg/darkfield_p5438/yamls/LP_788.yaml' #name of the template yaml file that we modify
...
param_dict = {
    'defect_amplitude': [2.0, 5.0, 10.0, 15.0], 'defect_lambda': [0.5, 1.0, 2.0, 5.0 ], 
}
...
for i, combo in enumerate(combinations):
    ...
    if 'O1_size' in params:
        O1_s = params['O1_size']
        if O1_s==0:
            ip['O1']['in'] = 0 #if the size of A1 = 0 we put it out
        else:
            ip['O1']['in'] = 1
            ip['O1']['size'] = O1_s * 1e-6

    if 'O0_size' in params:
        O0_s = params['O0_size']
        if O0_s==0:
            ip['O0']['in'] = 0 #if the size of A1 = 0 we put it out
        else:
            ip['O0']['in'] = 1
            ip['O0']['size'] = O0_s * 1e-6
            

    if 'defect_amplitude' in params:
        defect_amp = params['defect_amplitude']        
        ip['O0']['defect_amplitude'] = defect_amp * 1e-6

    if 'defect_lambda' in params:
        defect_lamb = params['defect_lambda']        
        ip['O0']['defect_lambda'] = defect_lamb * 1e-6
    ...
    if 'BS_size' in params:
        BS_s = params['BS_size']
        if BS_s==0:
            ip['beam_shaper']['in'] = 0 #if the size of O2 = 0 we put it out
        else:
            ip['beam_shaper']['in'] = 1
            ip['beam_shaper']['size'] = BS_s * 1e-6
